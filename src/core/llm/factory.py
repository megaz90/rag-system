...
def llm_provider(**kwargs) -> BaseLLMChatModel:
    """
    Factory function that instantiates and returns an LLM provider
    based on the configured provider name.

    Additional keyword arguments are forwarded to the underlying
    provider class constructor, allowing flexible configuration
    (e.g., system prompts, API settings, etc.).
    """
